...
namespace Zyan.Communication.ChannelSinks.Counter
{
    ...
    internal class CounterServerChannelSink : BaseChannelSinkWithProperties, IServerChannelSink
    {
        ...
        /// <param name="responseHeaders">Anfrage-Header</param>
        ...
        /// <param name="requestStream">Antwort-Datenstrom</param>
        ...
        /// <param name="responseHeaders">Original Anfrage-Header</param>
        ...
        private void DoProcessMessageBefore(IMessage msg, ITransportHeaders requestHeaders, Stream requestStream)
        {
            ...
            Console.WriteLine(String.Format("Request: - ServerCounterBefore - ClientID - {0} - IP - {1} - Methode - {2} - {3} Bytes", requestHeaders[CommonTransportKeys.ConnectionId], requestHeaders[CommonTransportKeys.IPAddress], myIDictionary!=null?myIDictionary["__MethodName"]:"Unknown", length));
        }
        ...
        /// <param name="requestStream">Antwort Anfrage-Datenstrom (unverschlüsselt)</param>
        private void DoProcessMessageAfter(IMessage msg, ITransportHeaders responseHeaders, Stream responseStream, ITransportHeaders requestHeaders)
        {
            ...
            //Request zählen vom Server ausgeben
            Console.WriteLine(String.Format("Response: - ServerCounterAfter - ClientID - {0} - IP - {1} - Methode - {2} - {3} Bytes", requestHeaders[CommonTransportKeys.ConnectionId], requestHeaders[CommonTransportKeys.IPAddress], myIDictionary != null ? myIDictionary["__MethodName"] : "Unknown", length));
            ...
            if (myIDictionary != null && myIDictionary["__MethodName"].ToString().Equals("Invoke"))
            {
                ...
                foreach (object header in requestHeaders)
                {
                    ...
                    {
                        Console.WriteLine("Args: {0}", en.Value);
                    }
                }
                ////Argumente ermitteln
                //object[] args = myIDictionary["__OutArgs"] as object[];
                //if (args != null)
                //{
                   
                //    foreach (object arg in args)
                //    {
                //        Console.WriteLine("OutArgs: {0}", arg);
                //    }

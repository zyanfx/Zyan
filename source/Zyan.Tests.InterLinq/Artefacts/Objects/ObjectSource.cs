...
namespace InterLinq.UnitTests.Artefacts.Objects
{
	...
	public class ObjectSource : IObjectSource, IQueryHandler
	{
		...
		#region IQueryHandler Members (explicit implementation)

		IQueryable IQueryHandler.Get(Type type)
		{
		...
		IQueryable<T> IQueryHandler.Get<T>()
		{
			return this.Get<T>().AsQueryable();
		}

		bool IQueryHandler.StartSession()
		{
			return true;
		}

		bool IQueryHandler.CloseSession()
		{
			return true;
		}
